...
export default function ProjectsPage() {
  ...
  const getStatusBadge = (project: any) => {
    const isOverdue = project.expectedDeliveryDate &&
                      isPast(startOfDay(new Date(project.expectedDeliveryDate))) &&
  ...
  const getPaymentBadge = (payment: any) => {
    if (!payment) return <Badge variant="outline">Unknown</Badge>;
    if (payment.paymentStatus === "fully_paid") return <Badge variant="success">Paid</Badge>;
    if (payment.paymentStatus === "partially_paid") return <Badge variant="warning">Partial</Badge>;
    ...
    <DashboardLayout>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground mt-1">Manage and track project progress</p>
        </div>
        {role === "admin" && (
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredProjects.map((project) => {
                    const symbol = project.payment?.currency === "USD" ? "$" : project.payment?.currency === "EUR" ? "€" : "₹";
                    return (
                      <tr key={project._id} className="border-b border-border hover:bg-white/5 transition-colors">
                        <td className="px-6 py-4">
                          <div className="font-semibold flex items-center gap-2">
                            <span>#{project.sNo}</span> {project.clientName}
                          </div>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-col gap-1 items-start">
                            {getPaymentBadge(project.payment)}
                            <span className="text-xs text-muted-foreground font-mono">
                              {symbol}{project.payment?.amountReceived || 0} / {symbol}{project.projectBudget}
                            </span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
      ...
      <div className="md:hidden space-y-4">
        {loading ? (
          <div className="text-center text-muted-foreground py-8 border border-border rounded-lg bg-secondary/50">Loading projects...</div>
        ) : filteredProjects.length === 0 ? (
          <div className="text-center text-muted-foreground py-8 border border-border rounded-lg bg-secondary/50">No projects found.</div>
        ) : (
          filteredProjects.map((project) => {
            const symbol = project.payment?.currency === "USD" ? "$" : project.payment?.currency === "EUR" ? "€" : "₹";
            return (
              <Card key={project._id} className="overflow-hidden hover:border-primary/50 transition-colors">
                <Link href={`/projects/${project._id}`}>
                  <CardContent className="p-4 space-y-3">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <span className="text-xs font-mono text-muted-foreground">#{project.sNo}</span>
                        <h4 className="font-semibold text-lg leading-tight">{project.clientName}</h4>
                    ...
                    <div className="flex justify-between items-center bg-secondary/50 p-2 rounded-md">
                      ...
                      <div className="text-sm font-medium">
                        {symbol}{project.payment?.amountReceived || 0} <span className="text-muted-foreground font-normal text-xs">/ {symbol}{project.projectBudget}</span>
                      </div>
                    </div>

                    <div className="flex justify-between items-center text-xs text-muted-foreground pt-1">
                      <span>{format(new Date(project.startDate), "MMM dd, yy")}</span>
