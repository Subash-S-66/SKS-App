...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (!requirement) {
      return new NextResponse(JSON.stringify({ error: "Requirement not found" }), { status: 404 });
    }
    ...
    if (body.title && role === "admin") {
      requirement.title = body.title;
    }
    if (body.description && role === "admin") {
